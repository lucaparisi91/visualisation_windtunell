...
const renderParticles = function(initialPositions,velocityField)
{
    ...
    const scaleVelocity= (u,v ) => [u*paper.view.size.width/(xExtent[1]-xExtent[0]), v*paper.view.size.height/(yExtent[1]-yExtent[0])]
    ...
    paper.view.onFrame=function(event) {    
        ...
        for ( let i =0 ; i < nParticles ; i++)
        {
            const u = velocityField.u.interpolate(initialPositions[i][0],initialPositions[i][1])
            
            const v = velocityField.v.interpolate(initialPositions[i][0],initialPositions[i][1])    
            
            initialPositions[i][0]=pbc(initialPositions[i][0] + u*dt ,xExtent)
            initialPositions[i][1]=pbc(initialPositions[i][1] + v*dt ,yExtent)

            
            dots[i].position.x= xScale(initialPositions[i][0])
            dots[i].position.y= yScale(initialPositions[i][1])
...
window.onload = function() {
    ...
    fetch("/output.dat").then( (res) =>  { return res.blob() }   ).then( (blob) =>{ return blob.arrayBuffer()  }).then( (data) => { 
        ...
        renderParticles(initialPositions,field)
